...
def load_model():
    ...
    # Patch: create dummy class if missing
...
def apply_feature_engineering(df):
    ...
    df['AgeGroup'] = pd.cut(df['Age'], bins=[17, 30, 50, 70], labels=['Young', 'Adult', 'Senior'], right=True,
                            include_lowest=True)
...
if model:
    with st.form("user_input_form"):
        ...
        with c4:
            click_through_rate = st.number_input("Click Through Rate (CTR)", min_value=0.0, max_value=1.0, value=0.12,
                                                 step=0.01, format="%.2f")
            conversion_rate = st.number_input("Conversion Rate", min_value=0.0, max_value=1.0, value=0.08, step=0.01,
                                              format="%.2f")
